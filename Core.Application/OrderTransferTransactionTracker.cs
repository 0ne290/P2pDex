...
public class OrderTransferTransactionTracker : IDisposable
{
    ...
    private async void Handler(object? _, ElapsedEventArgs __) => await ExecuteConcurrentlyAsync(async () =>
    {
        ...
        foreach (var trackedSellOrder in await _unitOfWork.Repository.GetAllBy<Domain.Entities.SellOrder>(o => o.Status == OrderStatus.ReceiptFiatFromBuyerConfirmedBySeller))
            if (await HandleExchangerToBuyerTransferTransaction(trackedSellOrder))
                updatedSellOrders.Add(trackedSellOrder);
        
        /*foreach (var trackedBuyOrder in await _unitOfWork.Repository.GetAllBy<Domain.Entities.BuyOrder>(o => o.Status == OrderStatus.RespondedBySeller))
        ...
        foreach (var trackedBuyOrder in await _unitOfWork.Repository.GetAllBy<Domain.Entities.BuyOrder>(o => o.Status == OrderStatus.ReceiptFiatFromBuyerConfirmedBySeller))
